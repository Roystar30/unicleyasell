# TODO: Remove View Modes and Create Responsive Default View

- [x] Remove view buttons from index.html
- [x] Remove viewMode variable and applyViewMode function from script.js
- [x] Remove view-related event listeners from script.js
- [x] Remove view classes from stylesheet.css
- [x] Update .grid CSS for fully responsive default view
- [x] Test responsiveness on different screen sizes (user will test)
- [ ] chips style should change from flex to constant